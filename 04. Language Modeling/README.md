...
*   **Performance note:** The WKV has a per-channel decay, so it has no matmul shortcut and is the heaviest model here. It is computed in parallel "training mode" (a single batched `T×T` op, validated identical to the sequential recurrence — not the slow Python loop). This runs efficiently on a CUDA GPU. On Apple Silicon, prefer CPU (`--device cpu`) — for this small model CPU is ~3× faster than MPS, which handles the large elementwise `exp`/reduction poorly — and use `--limit` for quick iteration.
